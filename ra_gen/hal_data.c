...
/* Actual period: 0.000262144 seconds. Actual duty: 50%. */.period_counts = (uint32_t) 0x10000,
...
{ .mode = TIMER_MODE_PERIODIC,
/* Actual period: 0.001 seconds. Actual duty: 50%. */.period_counts = (uint32_t) 0x3d090,
  .duty_cycle_counts = 0x1e848, .source_div = (timer_source_div_t) 0, .channel = 6, .p_callback = NULL,
  /** If NULL then do not add & */
#if defined(NULL)
    .p_context           = NULL,
#else
...
const i2c_master_instance_t g_i2c_master_for_peripheral =
        { .p_ctrl = &g_i2c_master_for_peripheral_ctrl, .p_cfg = &g_i2c_master_for_peripheral_cfg, .p_api =
                  &g_i2c_master_on_iic };
